...
enum ScanButton_States { ScanButton_Init, ScanButton_Idle, ScanButton_Press, ScanButton_Release } ScanButton_State;

int ScanButtonTick(int ScanButton_State)
{
   //State Transitions
   switch(ScanButton_State)
   {
	case ScanButton_Init:
	if (power == On)
	{
	   ScanButton_State = ScanButton_Idle;
	}
	printf("SCAN BUTTON INIT");
	break;

	case ScanButton_Idle:
	if (power == Off)
        {
            ScanButton_State = ScanButton_Init;
        }
        if ((digitalRead(ScanButton) == PRESSED) && power == On)
        {
            ScanButton_State = ScanButton_Press;
        }
	printf("SCAN BUTTON IDLE");
	break;

	case ScanButton_Press:
	if (digitalRead(ScanButton) == RELEASED)
        {
            ScanButton_State = ScanButton_Release;
        }
	printf("SCAN BUTTON PRESS");
	break;

	case ScanButton_Release:
	ScanButton_State = ScanButton_Idle;
	printf("SCAN BUTTON RELEASE");
	break;
   ...
   switch(ScanButton_State)
   {
	...
	if(power == On)
	{
	   printf("Scan in your item!");
	   ssd1306_clearDisplay();
	   delay(100);
	   ssd1306_drawString("Scan in your item!");
	   ssd1306_display();
	   scanf("%12s", barcode);
	   printf("%s", barcode);
	}
...
int OledTick(int Oled_State)
{
	...
	switch (Oled_State)
	{
		...
		{
		   case Grams:
		   strcat(displayBuffer, "Grams: ");
		   break;

		   case Ounces:
		   strcat(displayBuffer, "Ounces: ");
		   break;

		   case Pounds:
		   strcat(displayBuffer, "Pounds: ");
		   break;
		...
		strcat(displayBuffer, weightBuffer);
		strcat(displayBuffer, "\n\n");

		//Append current wifi status using ping.c (NEEDS TESTING)
		strcat(displayBuffer, get_wifi_status());

		ssd1306_clearDisplay();
		ssd1306_drawString(displayBuffer);
		ssd1306_display();
		printf("%s\n", displayBuffer);
...
int main(void)
{
	...
	pullUpDnControl(ScanButton, PUD_UP);
	unsigned char scan_state = ScanButton_Init;
	printf("Scan Button Initialized!\n");
    ...
    while (1) 
    {
       ...
       scan_state = ScanButtonTick(scan_state);
